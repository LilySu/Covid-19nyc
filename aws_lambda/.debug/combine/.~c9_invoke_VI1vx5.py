...
def combine_counties_scraped_and_historical():
    ...
    # import datetime
    df['lastupdated'] = df['lastupdated'] - timedelta(days=1)
    ...
    for i in collist:
        # df_confirmed[i] = df_confirmed[i].astype(str)
        # df_confirmed[i] = df_confirmed[i].str.replace(' ', '')
        # df_confirmed[i] = df_confirmed[i].str.replace(',', '')
        df_confirmed[i] = df_confirmed[i].fillna(0)
        df_confirmed[i] = df_confirmed[i].astype('float64')
    del df
