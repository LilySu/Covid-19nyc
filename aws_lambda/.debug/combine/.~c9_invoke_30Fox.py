...
def combine_counties_scraped_and_historical():
    ...
    df.loc[:,'now'] = datetime.now(tz=pytz.timezone('EST'))
    df.loc[:,'now'] = df['now'].apply(lambda x: x.strftime('%B %d %H'))
    df = df[df['lastupdate']==df['now']]
    ...
    df_confirmed_historical_T = df_confirmed_historical_T.drop_duplicates()
    ...
    df.loc[:,'date'] = datetime.now(tz=pytz.timezone('EST'))
    ...
    df_confirmed = pd.concat([df_confirmed,df_confirmed_historical_T])
    del df_confirmed_historical_T
    
    df_confirmed.drop(columns=['Status','New York City a'], inplace=True)
    
    cols = df_confirmed.columns.drop('date')
    df_confirmed[cols] = df_confirmed[cols].apply(pd.to_numeric, errors='coerce')
    
    df_for_total = df_confirmed.drop(columns = ['date','Queens', 'Kings','Richmond','Bronx'])
        
    df_confirmed['total'] = df_for_total.sum(axis=1)
    del df_for_total
    df_confirmed = df_confirmed.drop_duplicates()    
    # indexNames = df_confirmed[ (df_confirmed['New York'] != 92384) & (df_confirmed['date'] == 'April 10') ].index
    # df_confirmed.drop(indexNames , inplace=True)
    df_confirmed.drop_duplicates(subset='date', keep="last")
    df_confirmed.sort_values(by='New York', ascending=False)
