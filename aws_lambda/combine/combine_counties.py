...
from sql_queries.sql_counties import get_scraped_counties # gets df, df_confirmed_historical_T

def combine_counties_scraped_and_historical():
    """ Returns a df with confirmed cases by county, date in the format of Month-day like 
    April 08, and a total"""

    df = get_scraped_counties()# get info from database
    df = pd.DataFrame(data, columns=['County', 'Confirmed', 'Deaths', 'Recoveries', 'Population','Deaths2Confirmed', 'Confirmed2Population','lastupdated'])
    
    df_confirmed_historical_T = get_combined_counties()# get from database
    df_confirmed_historical_T = pd.DataFrame(data, columns=['date', 'Albany', 'Allegany', 'Bronx', 'Broome', 'Cattaraugus',
